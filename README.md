...
- **문서 보관/해제 + 활동 로그** — 보관(읽기 전용 잠금)·해제는 **소유자·편집자만**(뷰어 차단). 보관/해제할 때마다 "누가·언제·무엇을"을 **append-only로 영구 기록**(`doc_activity`)하고, 문서 상단 배너에 "🔓 다시 열림 · 이름 · 시각" + 전체 이력으로 노출 — 몰래 다시 여는 행위를 추적한다.
- **자동 파생 기록** — 회의록(대화 날짜별)·릴리스(증류 시 append-only 스냅샷)를 뷰 + `.md`로.
- **접근 보안(멤버 전용)** — 문서·첨부의 모든 읽기·쓰기는 **프로젝트 멤버만**(비멤버는 404). 첨부는 이미지 외 다운로드 강제 + `nosniff`로 저장형 XSS 차단. 운영에서는 `SESSION_SECRET` 필수(미설정 시 부팅 실패).
...
- **`lib/repo.ts`** — 유일한 데이터 접근 경로. 모든 불변식이 여기서 강제됩니다. 문서 접근 게이트(`requireDocAccess` — 비멤버 거부)와 권한 판정(`getDocPermission` — owner/editor/viewer)도 여기서 제공합니다.
...
6. **문서 접근 게이트** — 프로젝트 문서의 모든 읽기·쓰기는 멤버만(`repo.requireDocAccess`, 비멤버는 404). 보관/해제는 소유자·편집자만이며 실행 시 `doc_activity`에 append-only로 기록된다.
...
npm run lint     # ESLint
npm test         # Vitest (결정적 로직 32개 — 캐시·분류·합의·입장승인·세션·문서접근게이트·보관활동로그 등)
...
│  ├─ doc/                   # DocLensShell, ArchiveButton, DocActivityBanner, LevelSelector
...
> 완료됨(v0.4): 문서 접근 보안(멤버 전용 게이트·첨부 IDOR/XSS 차단·세션 시크릿 필수화) · 보관/해제 권한 분리 + 활동 감사 로그.
...
- AI 키·세션 시크릿은 배포 후 환경변수로 주입(`gcloud run services update ... --update-env-vars`). **`SESSION_SECRET`은 운영에서 필수** — 미설정 시 세션 처리 시점에 앱이 즉시 실패한다(공개 기본키로 인한 세션 위조 방지).
